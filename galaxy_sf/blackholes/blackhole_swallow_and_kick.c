...
struct INPUT_STRUCT_NAME
{
    ...
    MyFloat MSP_AgeInMyr[CLUSTER_SINK_NUMMSP]; 
    MyFloat MSP_Metallicity[CLUSTER_SINK_NUMMSP]; 
#endif  
}
...
//    MyDouble accreted_Phi;
    ...
    int ThisTask; // record the task from which this data came from
    ...
    // define arrays to gather information on the MSPs that need to be appended
    struct cluster_sink_multiple_stellar_population append_MSP[CLUSTER_SINK_NUMMSP_ACCRETE]; 
#endif
#endif
}
...
//    ASSIGN_ADD_PRESET(BlackholeTempInfo[target].accreted_Phi, out->accreted_Phi, mode);
    ...
    for(k=0;k<NUM_METAL_SPECIES;k++) {
        ASSIGN_ADD_PRESET(BlackholeTempInfo[target].accreted_MetalMass[k], out->accreted_MetalMass[k], mode);
    }
    ...
    if (BlackholeTempInfo[target].flag_SinkMerger_withMSP > 0){
        for(k=0;k<CLUSTER_SINK_NUMMSP;k++){ // loop over MSPs in the ngb
            ...
            ASSIGN_ADD_PRESET(BlackholeTempInfo[target].combined_MSP[k].InitialAge, out->combined_MSP[k].InitialAge, mode);
            ...
            ASSIGN_ADD_PRESET(BlackholeTempInfo[target].combined_MSP[k].InitialMetallicity_Z, out->combined_MSP[k].InitialMetallicity_Z, mode);
#ifdef CLUSTER_SINK_OUTPUT_NUMSNE
            ASSIGN_ADD_PRESET(BlackholeTempInfo[target].combined_MSP[k].CumNumSNe, out->combined_MSP[k].CumNumSNe, mode);
            ASSIGN_ADD_PRESET(BlackholeTempInfo[target].combined_MSP[k].CumNumSNII, out->combined_MSP[k].CumNumSNII, mode);
            ASSIGN_ADD_PRESET(BlackholeTempInfo[target].combined_MSP[k].CumNumSNIa, out->combined_MSP[k].CumNumSNIa, mode);
#endif
#ifdef CLUSTER_SINK_DEBUG
            if ((P[i].ID == DEBUG_ID) && ((out->combined_MSP[k].Mass > 0) || (out->combined_MSP[k].Age > 0))){
                printf("[DEBUG - swallow - out2() - combine] ThisTask %d, out %d - target %d, mode %d - k %d - combined_MSP_Mass[k] %g, out %g - combined_MSP_Age[k] %g, out %g init %g - combined SNII %g SNIa %g \n", ThisTask, out->ThisTask,
                target, mode, k, BlackholeTempInfo[target].combined_MSP[k].Mass, out->combined_MSP[k].Mass, 
                BlackholeTempInfo[target].combined_MSP[k].Age, out->combined_MSP[k].Age, out->combined_MSP[k].InitialAge, 
                BlackholeTempInfo[target].combined_MSP[k].CumNumSNII, BlackholeTempInfo[target].combined_MSP[k].CumNumSNIa);
            }
#endif
				...
				out.accreted_photon_energy += photon_energy;
                    ...
                    if(P[j].Type == 5)  /* this is a BH-BH merger */
                    {
                        ...
                        for(k=0;k<CLUSTER_SINK_NUMMSP;k++){ // loop over MSPs in the ngb
...
#ifdef CLUSTER_SINK_DEBUG
                            if(ThisTask==37){
                                printf("[DEBUG - swallow - evaluate] *WE HAVE A MERGER with %d MSPs!* - ThisTask %d - eating ID %d - MSP %d - Initial/Current Mass %g %g, Age %g, age_ngb_inmyr %g zh_ngb %g\n",
                             num_msps, ThisTask, P[j].ID, k, P[j].MSP[k].InitialMass, P[j].MSP[k].Mass, P[j].MSP[k].Age, age_ngb_inmyr, zh_ngb);
                            }
#endif
...
#ifdef CLUSTER_SINK_DEBUG
                                if(ThisTask==37){
                                    printf("[DEBUG - swallow - evaluate] ThisTask %d - main sink: MSP i %d age %g zh %g %g -- ngb: ID %d, age ngb %g, age difference %g, zh %g %g zh difference %g\n",
                                    ThisTask, i, 
                                    ...
                                    zh_ngb, zh_ngb/All.SolarAbundances[0], fabs(log10(zh_ngb/All.SolarAbundances[0]) - log10(local.MSP_Metallicity[i]/All.SolarAbundances[0])));
                                }
#endif
                            ...
                            if (idx_msp_main_sink < 0){ 
                                for(int i=0;i<CLUSTER_SINK_NUMMSP_ACCRETE;i++){ // loop over the MSPs in the output structure
                                    if(out.append_MSP[i].InitialMass == 0){idx_msp_to_append = i; break;}
                                }   
                            }

#ifdef CLUSTER_SINK_DEBUG
                            if(ThisTask==37){
                                printf("[DEBUG - swallow - evaluate] ThisTask %d - ngb ID %d, age %g, z %g - idx_msp_main_sink %d, idx_msp_to_append %d\n", ThisTask, P[j].ID, age_ngb_inmyr, zh_ngb, idx_msp_main_sink, idx_msp_to_append);
                            }
#endif
                            assert(idx_msp_main_sink*idx_msp_to_append <= 0); // MRC - if they're both still negative, something is wrong
                            ...
                            if(idx_msp_main_sink >= 0){ // combine as a mass-weighting
                                ...
                                out.combined_MSP[idx_msp_main_sink].InitialAge = FLT(P[j].MSP[k].InitialAge); // we do not mass-weight it
                                ...
                                out.combined_MSP[idx_msp_main_sink].InitialMetallicity_Z = FLT(P[j].MSP[k].InitialMetallicity_Z); // we do not mass-weight it
#ifdef CLUSTER_SINK_OUTPUT_NUMSNE
                                out.combined_MSP[idx_msp_main_sink].CumNumSNe += FLT(P[j].MSP[k].CumNumSNe);
                                out.combined_MSP[idx_msp_main_sink].CumNumSNII += FLT(P[j].MSP[k].CumNumSNII);
                                out.combined_MSP[idx_msp_main_sink].CumNumSNIa += FLT(P[j].MSP[k].CumNumSNIa);
#endif
                                ...
                                out.append_MSP[idx_msp_to_append].InitialMass = FLT(P[j].MSP[k].InitialMass);
                                out.append_MSP[idx_msp_to_append].Age = FLT(P[j].MSP[k].Age);
                                out.append_MSP[idx_msp_to_append].InitialAge = FLT(P[j].MSP[k].InitialAge);
                                assert(out.append_MSP[idx_msp_to_append].Age <= All.Time);
                                assert(out.append_MSP[idx_msp_to_append].Age > 0); // tform = 0 make no sense
                                for(int l=0;l<NUM_METAL_SPECIES;l++){
                                    out.append_MSP[idx_msp_to_append].Metallicity[l] = FLT(P[j].MSP[k].Metallicity[l]);
                                    assert(out.append_MSP[idx_msp_to_append].Metallicity[l] <= 1);
                                    assert(out.append_MSP[idx_msp_to_append].Metallicity[l] > 0);
                                }
                                out.append_MSP[idx_msp_to_append].InitialMetallicity_Z = FLT(P[j].MSP[k].InitialMetallicity_Z);
#ifdef CLUSTER_SINK_OUTPUT_NUMSNE
                                out.append_MSP[idx_msp_to_append].CumNumSNe = FLT(P[j].MSP[k].CumNumSNe);
                                out.append_MSP[idx_msp_to_append].CumNumSNII = FLT(P[j].MSP[k].CumNumSNII);
                                out.append_MSP[idx_msp_to_append].CumNumSNIa = FLT(P[j].MSP[k].CumNumSNIa);
#endif
                           }
                        }
#endif
